...
employ_data_path='/ossfs/workspace/proj/work_llm/work_data_monthly/EMP_TEMP_SEX_AGE_GEO_NB_M-20241121T1425.csv'
...
plt.title("Prophet Model - Employment Forecast")
plt.xlabel("Date")
plt.ylabel("Employment")
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/Prophet_Employment_Forecast.png")
plt.show()

# Step 10: 组件分析（趋势、季节性等）
fig_components = prophet_model.plot_components(forecast)
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/Prophet_Employment_Components.png")
plt.show()
...
plt.title('Impact of LLM Scores on Employment (DID Regression)')
plt.ylabel('Coefficient Value (Scaled)')
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/DID_Impact_LLM_Scores.png")
plt.show()
...
plt.title('Impact of LLM Scores on Employment (IV Regression)')
plt.ylabel('Coefficient Value (Scaled)')
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/IV_Impact_LLM_Scores.png")
plt.show()
...
fig.set_size_inches(12, 12)  # 设置图形大小
plt.title('Dynamic Impact of LLM Scores on Employment with Lags 5')
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/Dynamic_Impact_LLM_Scores_5.png")
plt.show()
...
fig_lag.set_size_inches(12, 12)  # 设置图形大小
plt.title('Dynamic Impact of LLM Scores on Employment with Lags 10')
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/Dynamic_Impact_LLM_Scores_10.png")
plt.show()
...
# 绘制就业数据和模型分数
plt.plot(merged_data.index, merged_data['obs_value'], label='Observed Employment')
plt.plot(merged_data.index, merged_data['Score'] * 1000, label='LLM Scores (Scaled)', linestyle='--')
...
# 图表标题和标签
plt.title('Observed Employment and LLM Scores Over Time')
plt.xlabel('Date')
plt.ylabel('Employment')
plt.legend()
...
# 保存图表
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/Observed_Employment_and_LLM_Scores.png")
plt.show()
...
future_data['Predicted_Employment'] = did_model.predict(X_future)

# 可视化更长时间的预测结果
plt.figure(figsize=(12, 6))
plt.plot(future_data['Date'], future_data['Predicted_Employment'], label='Predicted Employment (Long-term)', linestyle='--', color='orange')
plt.plot(merged_data.index, merged_data['obs_value'], label='Observed Employment', color='blue')
plt.title("Long-term Employment Prediction with LLM Score Growth")
plt.xlabel("Date")
plt.ylabel("Employment")
plt.legend()
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_employment/Long_Term_Employment_Prediction.png")
plt.show()
