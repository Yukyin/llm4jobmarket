...
employ_data_path='/ossfs/workspace/proj/work_llm/work_data_monthly/EAR_XEES_SEX_ECO_NB_M-20241121T1519.csv'
...
employment_data.to_csv("/ossfs/workspace/proj/work_llm/work_data_monthly/processed_earning.csv")
...
plt.title("Prophet Model - Earning Forecast")
plt.xlabel("Date")
plt.ylabel("Earning")
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/Prophet_Earning_Forecast.png")
plt.show()

# Step 10: 组件分析（趋势、季节性等）
fig_components = prophet_model.plot_components(forecast)
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/Prophet_Earning_Components.png")
plt.show()
...
plt.title('Impact of LLM Scores on Earning (DID Regression)')
plt.ylabel('Coefficient Value (Scaled)')
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/DID_Impact_LLM_Scores.png")
plt.show()
...
plt.title('Impact of LLM Scores on Earning (IV Regression)')
plt.ylabel('Coefficient Value (Scaled)')
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/IV_Impact_LLM_Scores.png")
plt.show()
...
plt.title('Dynamic Impact of LLM Scores on Earning with Lags 5')
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/Dynamic_Impact_LLM_Scores_5.png")
plt.show()
...
plt.title('Dynamic Impact of LLM Scores on Earning with Lags 10')
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/Dynamic_Impact_LLM_Scores_10.png")
plt.show()
...
# 绘制就业数据和模型分数
plt.plot(merged_data.index, merged_data['obs_value'], label='Observed Earning')
...
# 图表标题和标签
plt.title('Observed Earning and LLM Scores Over Time')
plt.xlabel('Date')
plt.ylabel('Earning')
plt.legend()
...
# 保存图表
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/Observed_Earning_and_LLM_Scores.png")
plt.show()
...
future_data['Predicted_Earning'] = did_model.predict(X_future)

# 可视化更长时间的预测结果
plt.figure(figsize=(12, 6))
plt.plot(future_data['Date'], future_data['Predicted_Earning'], label='Predicted Earning (Long-term)', linestyle='--', color='orange')
plt.plot(merged_data.index, merged_data['obs_value'], label='Observed Earning', color='blue')
plt.title("Long-term Earning Prediction with LLM Score Growth")
plt.xlabel("Date")
plt.ylabel("Earning")
plt.legend()
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_earning/Long_Term_Earning_Prediction.png")
plt.show()
