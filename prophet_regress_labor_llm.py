...
employ_data_path='/ossfs/workspace/proj/work_llm/work_data_monthly/EAP_DWAP_SEX_AGE_GEO_RT_M-20241121T1522.csv'
...
plt.title("Prophet Model - Labor Forecast")
plt.xlabel("Date")
plt.ylabel("Labor")
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/Prophet_Labor_Forecast.png")
plt.show()

# Step 10: 组件分析（趋势、季节性等）
fig_components = prophet_model.plot_components(forecast)
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/Prophet_Labor_Components.png")
plt.show()
...
plt.title('Impact of LLM Scores on Labor (DID Regression)')
plt.ylabel('Coefficient Value (Scaled)')
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/DID_Impact_LLM_Scores.png")
plt.show()
...
plt.title('Impact of LLM Scores on Labor (IV Regression)')
plt.ylabel('Coefficient Value (Scaled)')
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/IV_Impact_LLM_Scores.png")
plt.show()
...
plt.title('Dynamic Impact of LLM Scores on Labor with Lags 5')
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/Dynamic_Impact_LLM_Scores_5.png")
plt.show()
...
plt.title('Dynamic Impact of LLM Scores on Labor with Lags 10')
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/Dynamic_Impact_LLM_Scores_10.png")
plt.show()
...
# 绘制就业数据和模型分数
plt.plot(merged_data.index, merged_data['obs_value'], label='Observed Labor')
...
# 图表标题和标签
plt.title('Observed Labor and LLM Scores Over Time')
plt.xlabel('Date')
plt.ylabel('Labor')
plt.legend()
...
# 保存图表
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/Observed_Labor_and_LLM_Scores.png")
plt.show()
...
future_data['Predicted_Labor'] = did_model.predict(X_future)

# 可视化更长时间的预测结果
plt.figure(figsize=(12, 6))
plt.plot(future_data['Date'], future_data['Predicted_Labor'], label='Predicted Labor (Long-term)', linestyle='--', color='orange')
plt.plot(merged_data.index, merged_data['obs_value'], label='Observed Labor', color='blue')
plt.title("Long-term Labor Prediction with LLM Score Growth")
plt.xlabel("Date")
plt.ylabel("Labor")
plt.legend()
plt.tight_layout()
plt.savefig("/ossfs/workspace/proj/work_llm/result_labor/Long_Term_Labor_Prediction.png")
plt.show()
